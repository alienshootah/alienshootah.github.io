...
// Урок сделан по мотивам видео

const FPS = 60; // frames per second
const FRICTION = 0.7; // friction coefficien of space (0 = no friction, 1 = lots of friction)
const GAME_LIVES = 3; // starting number of lives
const LASER_MAX = 10; // maximum number of lasers on screen at once
const LASER_DIST = 0.6; // max distance laser can travel and fractin of screen width
const LASER_EXPLODE_DUR = 0.1; // duration of the lasers' blink during invisibility
const LASER_SPD = 500; // spped of lasers in pixels per second
const ROIDS_JAG = 0.4; // jagedness of the asteroids (0 = none, 1 = lots)
const ROIDS_PIS_LGE = 20; // points scored for a large asteroid
const ROIDS_PIS_MED = 50; // points scored for a medium asteroid
const ROIDS_PIS_SML = 100; // points scored for a small asteroid
const ROIDS_NUM = 3; // starting number of asteroids
const ROIDS_SIZE = 100; // starting size of asteroids in pixels
const ROIDS_SPD = 20; // max starting speed of asteroids in pixels per second
...
const OUTPUT_LEFT = 0; // expected neural outpur for turning left
const OUTPUT_RIGHT = 1; // expected neural outpur for turning right
const OUTPUT_THRESHOLD = 0.25; // how close the prediction must be to commit to a turn
const RATE_OF_FIRE = 5; // shots per second
...
var fxThrust = new Sound("sounds/thrust.m4a", 5, 0.5);
...
if (AUTOMATION_ON) {
    ...
    for (let i = 0; i < NUM_SAMPLES; i++) {
        ...
        // random asteroid location (include of-screen data)
        ...
        // train the network
        nn.train(normalliseInput(ax, ay, angle, sa), [direction]);
    }
}
...
function newAsteroid(x, y, r) {
    ...
    var roid = {
        ...
        r: r,
        a: Math.random() * Math.PI * 2, // in radiants
...
function normalliseInput(roidX, roidY, roidA, shipA) {
...
function update() {
    ...
    if (AUTOMATION_ON) {
        ...
        let sy = ship.y;
        let angle = angleToPoint(sx, sy, sa, ax, ay);
        let predict = nn.feedForward(normalliseInput(ax, ay, angle, sa)).data[0][0];
    ...
    if (ship.thrusting && !ship.dead) {
        ...
        if (!exploding && blinkOn) {
            ...
            ctx.lineTo( // rear centre behhind the ship
