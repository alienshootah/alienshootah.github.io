...
class NeuralNetwork {
    constructor(numInputs, numHidden, numOutputs) {
        ...
        this._weight0 = new Matrix(this._numInputs, this._numHidden);
        this._weight1 = new Matrix(this._numHidden, this._numOutputs);
        ...
        this._weight0.randomWeights();
        this._weight1.randomWeights();
    }
    ...
    feedForward(inputArray) {
        ...
        this.hidden = Matrix.dot(this.inputs, this.weight0);
        ...
        let outputs = Matrix.dot(this.hidden, this.weight1);
    ...
    train(inputArray, targetArray) {
        ...
        // calculate hidden layer orrors (deltas "dot" transpose of weights1)
        let weights1T = Matrix.transpose(this.weight1);
        ...
        this.weight1 = Matrix.add(this.weight1, Matrix.dot(hiddenT, outputDeltas));
        let inputsT = Matrix.transpose(this.inputs);
        this.weight0 = Matrix.add(this.weight0, Matrix.dot(inputsT, hiddenDeltas));
...
class Matrix {
    ...
    // check metrices have the same dimensions
    static checkDimensions(m0, m1) {
        if (m0.rows != m1.rows || m0.cols != m1.cols) {
            throw new Error("Matrices are of different dientions!");
        }
    }
